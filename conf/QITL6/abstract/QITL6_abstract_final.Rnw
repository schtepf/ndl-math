...
\todo{Stefan: distinguish between convergence of a particular learner and stochastic convergence of $E[V_j]$; batch updates should be equivalent to the expectation(?)}
...
\end{equation}
The least-squares solution must satisfy the condition $\nabla E(\mathbf{w}) = \mathbf{0}$, which leads to the so-called normal equations \todo{cite reference (textbook)}
\begin{equation}
  \label{eq:lsr-normal}
  \mathbf{X}^T \mathbf{X} \mathbf{w} = \mathbf{X}^T \mathbf{z} .
\end{equation}
In the case of the W-H rule, $\mathbf{X}$ is a coincidence matrix between cues and events, with $x_{ti} \in \{0, 1\}$.  A straightforward calculation shows that $\mathbf{X}^T \mathbf{X}$ is a square matrix of co-occurrence counts $f(C_i, C_j)$ between cues, and $\mathbf{X}^T \mathbf{z}$ is a vector of co-occurrence counts $f(C_i, O)$ between the cues and the outcome $O$.  \todo{Stefan: spell out the ``straightforward'' calculation?}  Dividing Eq.~(\ref{eq:lsr-normal}) by $m$, we obtain Eq.~(\ref{eq:danks-equi}) with $\lambda = 1$, i.e.
\[
...
\todo{Stefan: note on the case where $\mathbf{X}$ is rank-deficient; linear regression chooses the minimum solution, but what about R-W? undefined behaviour? we could make conjectures here and propose simulation studies as future work}

\section{R-W and logistic regression}
\label{sec:logistic}

\todo{Stefan: add short section on logistic regression, which is generally more sensible for category predictions than least-squares; gradient descent leads to update equation analogous to R-W, which we could also simulate}

\todo{Antti: some comments on experiments indicating that logistic regression learns implausible associations if there are too many cues/features; further investigation will be future work}
