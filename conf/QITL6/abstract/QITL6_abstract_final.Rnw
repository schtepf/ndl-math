...
\end{equation}
The least-squares solution must satisfy the condition $\nabla E(\mathbf{w}) = \mathbf{0}$, which leads to the so-called normal equations \cite[649]{degroot2002}
\begin{equation}
  \label{eq:lsr-normal}
...
\bibitem{degroot2002}
M.~H. DeGroot and M.~J. Schervish, \emph{Probability and Statistics}, 3rd~ed.\ Boston: Addison Wesley, 2002.
