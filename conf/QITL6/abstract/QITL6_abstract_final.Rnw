...
While precise results on the correspondence of the R-W equations with a certain single-layer perceptron \cite[155f]{suttonbarto1981} and with least-squares regression \cite[457]{stone1986} are available, they are scattered across various publications, presented in different notations, and often given without full, self-contained derivations.  The most comprehensive treatment of the relation between R-W and various types of neural networks is a recent monograph by Dawson \cite{dawson2008}.  Dawson combines a mathematical comparison with various simulation experiments, but he rejects neural networks with a linear activation function as inappropriate \cite[11]{dawson2008} and consequently does not discuss the connection to least-squares regression at all.  
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
Here, $\alpha_i$ is a measure of the salience of cue $C_i$, $\beta_1$ and $\beta_2$ are learning rates for positive ($o=1$) and negative ($o=0$) events, and $\lambda$ is the maximal activation level of the outcome $O$.  A simplified form of the R-W equations proposed by \cite{widrowhoff1960} in the context of artificial neural networks assumes that $a_1 = \dots = a_n = 1$,  $\beta_1 = \beta_2 = \beta$ and $\lambda = 1$ (known as the Widrow-Hoff or W-H rule).

We will make the assumption that $\beta_1 = \beta_2 = \beta$ from here on, but put no restrictions on $a_i$ or $\lambda$.  The R-W equations can then be combined into a single formula
%%
\begin{equation}
  \label{eq:rw-single}
  \Delta V_i = c_i a_i \beta \left( \lambda o - \sum_{j=1}^n c_j V_j \right) .
\end{equation}
%%
...
\begin{equation}
  ...
  \begin{split}
    E[\Delta V_i\psupt]
    &= a_i\beta \left( \lambda E[o c_i] - \sum_{j=1}^n E[c_i c_j] E[V_j\psupt] \right) \\
    &= a_i\beta \left( \lambda P(O, C_i) - \sum_{j=1}^n P(C_i, C_j) E[V_j\psupt] \right) .
  \end{split}
\end{equation}
%% 
Note that $E[xy] = E[x]\cdot E[y]$ for independent random variables $x$ and $y$, that the expectation of the product $o c_i$ of two indicator variables is the co-occurrence probability of the two events, i.e.\ $E[o c_i] = P(O, C_i)$, and that $E[c_i c_j] = P(C_i, C_j)$ for the same reason.  Eq.~(\ref{eq:exp-rw}) is identical to \cite[(A-5)]{gluckbower1988} except for differences in notation.

Danks \cite{danks2003} argues that a successful R-W model should converge to an equilibrium state $\mathbf{V} = \lim_{t\to\infty} E[\mathbf{V}\psupt]$ of the expected association vector, where the expected update $E[\Delta \mathbf{V}] = 0$.  From Eq.~(\ref{eq:exp-rw}), we obtain the identity
\begin{equation}
  \label{eq:danks-equi}
  \lambda P(O, C_i) - \sum_{j=1}^n P(C_i, C_j) V_j = 0
\end{equation}
for the Danks equilibrium, which is equal to his Eq.~(11) \cite[113]{danks2003} multiplied by $P(C_i)$.
...
\end{equation}
The least-squares solution must satisfy the condition $\nabla E(\mathbf{w}) = \mathbf{0}$, which leads to the so-called normal equations \cite[142]{bishop2006}
\begin{equation}
  \label{eq:lsr-normal}
...
\end{equation}
%%
according to \cite[205]{bishop2006}.  Interpreting the weights $w_i$ as salience-adjusted associations $V_j = a_j w_j$ as in Sec.~\ref{sec:slp} and keeping in mind that $x_i = a_i c_i$, we obtain a form that is strikingly similar to the R-W equations.
%%
\begin{equation}
  \label{eq:lrm-rw}
  \Delta V_i =
...
Therefore, it would be particularly interesting to observe and compare in detail the behavior of such a logistic regression learner with the R-W one in terms of the incremental development of the cue-outcome associations as well as their asymptotes, using real linguistic data. General rules-of-thumb for logistic regression recommend at least 10 or 20 as many datapoints as variables (cue-outcome pairings) in order to gain a valid and reliable model (e.g. \cite{peduzzi1996}; \cite[116]{arppe2008}). Accordingly, prior experimental research \cite{arppebaayen2011} has indicated that if one increases the number of cues one observes too much in relation to the available datapoints, at some point the associations (as odds-ratios) in logistic regression, and the probability estimates based on these associations, become so extreme as to be practically useless. In contrast, the cue-outcome weights and probability estimates for corresponding NDL models remain relatively well-behaved despite increasing the observed cue/outcome-to-data ratio.
...
Having established NDL as linear regression, with its well-known drawbacks (e.g.\ a propensity for overfitting the training data, especially if there is a large number $n$ of cues), it will be interesting to contrast it with more state-of-the-art machine learning techniques, as a systematic follow-up and analysis of the empirical observations in \cite{arppebaayen2011}.\footnote{Results using the Finnish \textsc{think} dataset \cite{arppe2008, arppe2013} indicated that (1) outcomes predicted by NDL and GLM (in the case of the 4-way \emph{polytomous} outcome using the \emph{one-vs-rest} technique implemented in the \texttt{polytomous} R package \cite{arppe2013}) agree with a rate of 94.8\%, that (2) the distributions of instance-wise maximum NDL cumulative association weights and corresponding maximum GLM expected probabilities have similar general distribution contours (cf. \figurename ~\ref{think_assocs_probs}), and that (3) instance-wise maximum NDL association weights and corresponding maximum GLM expected probabilities correlate highly, with $r_{Spearman}=.950$, as do (4) individual NDL association weights and GLM log-odds, with $r_{Spearman}=.897$.} We plan to pursue further the mathematical analysis and empirical study, with  real linguistic datasets, of (i) logistic regression (a subtype of Generalized Linear Model [GLM]), which are considered more appropriate for categorical data than linear least-squares regression, and (ii) regularization techniques, which control overfitting and encourage sparse solutions. Finally, as NDL through the R-W equations is based on a model of human learning, an interesting avenue for further research would be to compare cue-outcome associations weights (based on applying NDL on corpus data, representing naturally produced language) with experiments that would hone in on the cognitive processing of the same linguistic stimuli by human participants.
