...
\todo{Antti: discuss different ``applications'' of R-W as model of item-by-item learning in simple controlled experiments or with focus on asymptotic/stochastic learning behaviour for complex, non-separable problems.  Explain why we believe the latter is of particular interest in linguistics and why we see it as a main application area of NDL. It is the focus of this paper.}
...
\todo{Antti: note that it will be interesting to compare this learner with the original R-W; some comments on experiments indicating that logistic regression learns implausible associations if there are too many cues/features; further investigation will be future work}
...
\todo{Antti: revise and extend future work section; especially things we want to look at in the next months}
